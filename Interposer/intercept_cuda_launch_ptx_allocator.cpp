...
/*	std::cerr << "Current partition allocations: "
	...
	//std::cerr<<"previous end ptr: "<<prev_end_ptr<<std::endl;
        ...
        if (prev_end_ptr == NULL)
            ...
            allocation.base_alloc_ptr = prev_end_ptr;
...
void createModuleMap() {
	...
	std::string path = "../Interposer/modified_cufft_11.0/" + ptx;  
...
void parseJson()
{
    std::string jsonFile = "../Interposer/klist_fft.json";
...
#endif
#ifdef DEBUG
    for (const auto& elem : ptr2CUfunc_param) {
	    std::cout << elem.first << " : (" << elem.second.first << ", " << elem.second.second << ")" << std::endl;
    }

    std::pair<CUfunction, int> result;
    int ret = lookup_value(ptr2CUfunc_param, func, result);
    if (ret == 0) {
	    std::cout << "Found key : (" << result.first << ", " << result.second << ")" << std::endl;
    } else {
	    std::cout << "Error: key not found in map" << std::endl;
    }
#endif
